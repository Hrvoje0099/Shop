...
//				item = null;
				...
				if (txtItem.getText().isEmpty())
					...
					// sprema samo na tablicu(ne u bazu još)
		...
		// ActionListener za naèin plaæanja KARTICOM gdje æemo spremiti i košaricu i blagajnu u bazu
		...
		// ActionListener za naèin plaæanja GOTOVINOM gdje æemo spremiti i košaricu i blagajnu u bazu
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if (Double.valueOf(txtAmountTotal.getText().toString().replaceAll(",", ".")) <= 0) {
						...
						JOptionPane.showMessageDialog(null, "UNOS GOTOVINE MORA BITI VEÆI OD IZNOSA RAÈUNA!", "GREŠKA", JOptionPane.ERROR_MESSAGE);
				} catch (Exception e1) {
					e1.printStackTrace(new PrintWriter(errors));
					JOptionPane.showMessageDialog(null, e1, "GREŠKA", JOptionPane.ERROR_MESSAGE);
		...
		// ActionListener za RAÈUNI button
		...
		// izbriši oznaèeni red u tablici kada stisnemo DELETE na tipkovnici
		...
		double quantity;
			if (item.getUnit().equals("KG")) {
				...
				if (!(Pattern.matches("^[0-9]+$", txtQuantity.getText()))) {
					JOptionPane.showMessageDialog(null, "KOLIÈINA NE MOŽE BITI DECIMALNA", "GREŠKA", JOptionPane.ERROR_MESSAGE);
				}
				else
	...
	private JMenuBar createMenuBar() {
		...
		menuBar.add(horizontalGlue);  //postavlja menu bar u desni ï¿½oï¿½ak
	...
	private void printBill(String paymentMethod) {
		...
		try {
			...
			Paragraph paragraphBillNumber = new Paragraph("Broj raèuna: " + txtBillNumber.getText(), f1);
			...
			for (int i = 0; i < 1; i++) {
				...
				table1.addCell(new PdfPCell(new Phrase("Kolièina", f2)));
			...
			Chunk chunkPaymentMethod = new Chunk("Naèin plaæanja: " + paymentMethod, f1);
			...
			Chunk chunkFooter = new Chunk("OTISNUTI RAÈUN JE PUNOVAŽAN BEZ POTPISA I PEÈATA SUKLADNO ODREDBAMA ZAKONA", f2);
