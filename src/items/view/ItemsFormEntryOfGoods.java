...
public class ItemsFormEntryOfGoods extends JPanel {
		...
		btnAmountInput.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (txtAmountInput.getText().equals("")) {
					JOptionPane.showMessageDialog(null, "MORATE UNIJENTI KOLIÈINU ULAZA ROBE!!", "GREŠKA", JOptionPane.ERROR_MESSAGE);
				} else if (!(Pattern.matches("^[0.0-9.9]+$", txtAmountInput.getText().replaceAll(",", ".")))) {
					JOptionPane.showMessageDialog(null, "KOLIÈINA ULAZA ROBE MOŽE SADRŽAVATI SAMO BROJEVE", "GREŠKA", JOptionPane.ERROR_MESSAGE);
				} else {
	...
	private void loadComponents() {
		...
		lblSupplier = new JLabel("Dobavljaè: ");
